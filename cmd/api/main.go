...
func setupRoutes(app *fiber.App, db *database.DB, rdb *database.RedisClient, cfg *config.Config) {
	...
	api.Get("/accounts/ledger-only", accountHandler.LedgerOnly)
	api.Get("/accounts", accountHandler.List)
	api.Post("/accounts/init-seed", accountHandler.InitFromSeed)
	api.Post("/accounts/auto-code", accountHandler.AutoCode)
	...
	api.Get("/expense-invoices/:id", expenseInvoiceHandler.GetByID)
	api.Put("/expense-invoices/:id", expenseInvoiceHandler.Update)
	...
	// Expense invoice OCR routes
	ocrSvc := service.NewOcrService("mock")
	expenseInvoiceOcrHandler := handler.NewExpenseInvoiceOcrHandler(ocrSvc)
	api.Post("/expense-invoices/ocr", expenseInvoiceOcrHandler.Recognize)
	...
	api.Get("/payroll/export-tax", payrollHandler.ExportTax)
	...
	api.Post("/periods", periodHandler.CreatePeriod)
	api.Put("/periods/:id", periodHandler.UpdatePeriod)
	api.Delete("/periods/:id", periodHandler.DeletePeriod)
	api.Post("/periods/:id/enable", periodHandler.EnablePeriod)
	...
	reconciliationSvc := service.NewReconciliationService(db.GetPool(), bankTransactionRepo, paymentRepo, arInvoiceRepo, apInvoiceRepo, invoiceRepo, reconRepo, journalRepo)
	...
	api.Get("/reconciliation/unmatched-summary", reconciliationHandler.GetUnmatchedSummary)
	...
	api.Post("/reconciliation/precheck/force-pass", reconciliationHandler.ForcePass)
	api.Post("/reconciliation/execute", reconciliationHandler.ExecutePairs)
	api.Post("/reconciliation/approve", reconciliationHandler.ApprovePairs)
	api.Post("/reconciliation/reject", reconciliationHandler.RejectPairs)
	...
	api.Get("/advance-receipts/summary", advanceReceiptHandler.GetSummary)
	...
	api.Get("/advance-payments/summary", advancePaymentHandler.GetSummary)
