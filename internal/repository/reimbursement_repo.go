...
func (r *ReimbursementRepository) Create(ctx context.Context, tenantID uuid.UUID, reimbursement *model.BusReimbursement) (*model.BusReimbursement, error) {
	query := `
		INSERT INTO bus_reimbursements (id, reimbursement_no, employee_name, department, expense_type, amount, posting_date, description, bank_account, docstatus, voucher_id, voucher_no, created_by, tenant_id, company_id)
	...
	err := r.pool.QueryRow(ctx, query,
		...
		reimbursement.Department, reimbursement.ExpenseType, reimbursement.Amount,
...
func (r *ReimbursementRepository) GetByID(ctx context.Context, tenantID uuid.UUID, id uuid.UUID) (*model.BusReimbursement, error) {
	query := `
		SELECT id, reimbursement_no, employee_name, department, expense_type, amount, posting_date, description, bank_account, docstatus, voucher_id, voucher_no, created_by, tenant_id, company_id, created_at
	...
	err := r.pool.QueryRow(ctx, query, id, tenantID).Scan(
		...
		&reim.ExpenseType, &reim.Amount, &reim.PostingDate, &reim.Description,
		&reim.BankAccount, &reim.DocStatus, &reim.VoucherID, &reim.VoucherNo,
		&reim.CreatedBy, &reim.TenantID, &reim.CompanyID, &reim.CreatedAt,
	)
	if err != nil {
