...
// GenerateFromPaymentEntry generates a voucher from a payment entry.
// POST /api/v1/payment-entries/:id/generate-voucher
func (h *VoucherAutoGenerateHandler) GenerateFromPaymentEntry(c *fiber.Ctx) error {
	tenantID := c.Locals("tenant_id").(uuid.UUID)

	paymentIDStr := c.Params("id")
	...
	je, err := h.svc.GenerateFromPaymentEntry(c.Context(), tenantID, paymentID, userID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	}
