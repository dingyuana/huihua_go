...
func (h *BankTransactionHandler) Import(c *fiber.Ctx) error {
	...
	autoGenFlag := strings.ToLower(c.Query("auto_generate_voucher"))
	autoGen := !(autoGenFlag == "false" || autoGenFlag == "0")
	...
	autoSkipped := []map[string]interface{}{}
	if autoGen && h.autoSvc != nil && len(result.ImportedTxns) > 0 {
		userID, _ := c.Locals("user_id").(uuid.UUID)
		if userID != uuid.Nil {
			for _, txn := range result.ImportedTxns {
				if txn.Classification == nil {
					continue
				}
				cls := *txn.Classification
				if !isAutoGenerable(cls) {
					continue
				}
				if reason := validateAutoGenTxn(cls, txn.Direction); reason != "" {
					autoSkipped = append(autoSkipped, map[string]interface{}{
						"bank_txn_id":    txn.ID,
						"txn_date":       txn.TxnDate,
						"description":    derefStr(txn.Description),
						"classification": cls,
						"debit":          txn.Debit.String(),
						"credit":         txn.Credit.String(),
						"direction":      derefStr(txn.Direction),
						"skip_reason":    reason,
					})
					continue
				}
	...
	log.Printf("Import auto_generate_voucher=%v, generated=%d, skipped=%d, errors=%v",
		autoGen, autoGeneratedCount, len(autoSkipped), autoGenErrors)
	...
	return c.JSON(fiber.Map{
		...
		"auto_generated_hint":  "为以下分类自动生成草稿凭证：银行费用、利息收入、税务费用、社保费用、保险费用、业务收款、业务付款（默认开启；传 ?auto_generate_voucher=false 可关闭）",
		"auto_created_parties": result.AutoCreatedParties,
		"auto_skipped":         len(autoSkipped),
		"auto_skipped_items":   autoSkipped,
	})
}
...
func isAutoGenerable(cls string) bool {
	switch cls {
...
func validateAutoGenTxn(cls string, direction *string) string {
	switch cls {
	case "tax_payment", "social_security", "insurance_fee", "business_payment":
		if direction == nil || *direction != "out" {
			...
			return fmt.Sprintf("direction mismatch: %s requires direction=%s, got %s", cls, want, got)
		}
	case "interest_income", "business_receipt":
