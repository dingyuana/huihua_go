...
func (h *BankTransactionHandler) Import(c *fiber.Ctx) error {
		...
		if userID != uuid.Nil {
			...
			if err == nil {
				for _, txn := range txns {
					...
					// Eligible for direct voucher generation (no need for confirmation)
					switch cls {
					case "bank_fee", "interest_income", "tax_payment", "social_security":
	...
	return c.JSON(fiber.Map{
		...
		"auto_generated_hint": "税务费用、银行费用、社保费用、利息收入已自动生成凭证",
	})
}
