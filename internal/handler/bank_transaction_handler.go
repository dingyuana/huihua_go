...
func (h *BankTransactionHandler) Import(c *fiber.Ctx) error {
	...
	// Auto-generate vouchers for eligible transactions (small-amount bank fees, interest income, tax payments)
	autoGeneratedCount := 0
	if h.autoSvc != nil && result.SuccessCount > 0 {
		userID, _ := c.Locals("user_id").(uuid.UUID)
		if userID != uuid.Nil {
			// Fetch just-imported unmatched transactions to find eligible ones
			...
			if err == nil {
				for _, txn := range txns {
					...
					isSmallFee := cls == "bank_fee" && txn.Credit.LessThanOrEqual(decimal.NewFromFloat(50))
					isInterest := cls == "interest_income"
					isTaxPayment := cls == "business_payment" && txn.Description != nil &&
						isTaxRelated(*txn.Description)

					if isSmallFee || isInterest || isTaxPayment {
	...
	log.Printf("Auto-generated %d vouchers from import", autoGeneratedCount)
