  ...
  <div class="diff-report-page">
    <div class="page-header">
      ...
      <div class="filter-bar">
        <el-select v-model="bankAccountId" placeholder="选择银行账户" style="width: 240px" filterable>
          <el-option v-for="acct in bankAccounts" :key="acct.id" :label="`${acct.bank_name} (${maskAccount(acct.account_number)})`" :value="acct.id" />
        </el-select>
        <el-input v-model="periodNo" placeholder="期间 YYYYMM" style="width: 140px; margin-left: 8px" />
        <el-button type="primary" :loading="loading" @click="loadReport" style="margin-left: 8px">生成报告</el-button>
        <el-button @click="exportCsv" :disabled="!report">导出 CSV</el-button>
      </div>
    </div>

    <el-row v-if="report" :gutter="16" class="summary">
      <el-col :span="6">
        <el-card shadow="never">
          <p class="lbl">银行对账单余额</p>
          <p class="val">¥{{ report.bank_balance }}</p>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never">
          <p class="lbl">企业日记账余额</p>
          <p class="val">¥{{ report.book_balance }}</p>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never">
          <p class="lbl">差额</p>
          <p class="val" :class="Number(report.difference) === 0 ? 'ok' : 'err'">¥{{ report.difference }}</p>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never" :class="report.adjusted_reconciled ? 'ok-bg' : 'err-bg'">
          <p class="lbl">调节后状态</p>
          <p class="val">{{ report.adjusted_reconciled ? '✅ 已平衡' : '❌ 不平衡' }}</p>
          <p class="ts">生成时间: {{ report.generated_at }}</p>
        </el-card>
      </el-col>
    </el-row>

    <el-row v-if="report" :gutter="16" style="margin-top: 16px">
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>
            <span class="card-title bank">🏦 银行有，企业无（{{ report.bank_only_count }} 笔，¥{{ report.bank_only_total }}）</span>
          </template>
          <el-table v-if="report.bank_only_items.length" :data="report.bank_only_items" size="small" border stripe>
            ...
            <el-table-column prop="direction" label="方向" width="60">
              <template #default="{ row }">
                <el-tag :type="row.direction === 'in' ? 'success' : 'danger'" size="small">{{ row.direction === 'in' ? '收' : '付' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="source_type" label="来源" width="80" />
          </el-table>
          <el-empty v-else description="无未达项" :image-size="48" />
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card shadow="never">
          ...
          <el-table v-if="report.book_only_items.length" :data="report.book_only_items" size="small" border stripe>
            <el-table-column prop="txn_date" label="日期" width="100" />
            <el-table-column prop="description" label="摘要" min-width="180" show-overflow-tooltip />
            <el-table-column prop="amount" label="金额" width="100" align="right" />
            <el-table-column prop="direction" label="方向" width="60">
    ...
    <el-empty v-if="!report && !loading" description="请选择银行账户和期间后点击「生成报告」" :image-size="80" />
  </div>
</template>
